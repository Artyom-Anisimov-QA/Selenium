...
from pages.base_page import BasePage, modal_window_selector, empty_modal_window_selector
...
def test_check_modal_window(browser):
    ...
    BasePage(browser).get_button_basket().click()
    assert BasePage(browser).find(modal_window_selector).is_displayed()
...
def test_check_text_in_dropdown(browser):
    ...
    MainPage(browser).open_url()
    BasePage(browser).get_button_basket().click()
    assert expected_text == BasePage(browser).find(empty_modal_window_selector).text
...
def test_pay_product(browser):
    ...
    BasePage(browser).get_button_basket().click() # Ищем кнопку Корзина
    BasePage(browser).checkout() # Кликаем по кнопке Checkout
