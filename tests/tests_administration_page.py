from pages.administration_page import AdminPage, login_selector, username_selector, passwd_selector, \
    password_elem_selector
...
def test_check_input_username_admin_page(browser):
    ...
    input_username = AdminPage(browser).find(login_selector)
...
def test_check_element_username_admin_page(browser):
    AdminPage(browser).open_url()
    element_username = AdminPage(browser).find(username_selector)
...
def test_check_input_password_admin_page(browser, base_url):
    AdminPage(browser).open_url()
    input_password = AdminPage(browser).find(passwd_selector)
...
def test_check_element_password_admin_page(browser, base_url):
    AdminPage(browser).open_url()
    element_username = AdminPage(browser).find(password_elem_selector)
    assert element_username.is_displayed()
